...
// get the PID from the given file, or by looking for "mind" process by name - returns -1 if process not running
...
pid_t getpid(std::string pidfile) {
    ...
    if (p > 1) {
        if (confirmname(p)) { // is that pid really MinD and running?
            return p; // it is so return it
        }
    }
    pid_t t = getpidfromcommand("mind"); // pid file method failed
...
int sysv_kill(std::string pidfile, bool dounlink) {
    ...
    std::cerr << "No MinD process found." << std::endl;
    return 1;
}
...
// show PID of running MinD process

int sysv_showpid(std::string pidfile) {
    pid_t p = getpid(pidfile);
    if (p > 1) {
        std::cout << "Parent MinD pid:" << p << std::endl;
        return 0;
    }
    std::cerr << "No MinD process found." << std::endl;
    return 1;
